...
//print_r($sql . '<br/>');
    ...
    public function getProduct($product_id) {
        ...
        $product_data = $this->cache->get('product.' . (int) $this->config->get('config_language_id') . '.' . (int) $this->config->get('config_store_id') . '.' . (int) $customer_group_id . '.' . (int) $product_id);
        ...
        if (!$product_data) {
            ...
            if ($query->num_rows) {
                ...
                $product_data = array(
                    ...
                    'brief_summary' => $query->row['brief_summary'],
                ...
                $this->cache->set('product.' . (int) $this->config->get('config_language_id') . '.' . (int) $this->config->get('config_store_id') . '.' . (int) $customer_group_id . '.' . (int) $product_id, $product_data);
            }
    ...
    public function getProducts($data = array()) {
        ...
        if (!$product_data) {
            ...
            if (!empty($data['filter_category_id']) || !empty($data['filter_name']) || !empty($data['filter_data']) || !empty($data['filter_filter']) || !empty($data['filter_option'])) {
                if (!empty($data['filter_sub_category'])) {
                ...
                foreach ($filter_groups as $key => $filters) {
                    if ($usingNamedFilters) {
                        $sql .= " LEFT JOIN " . DB_PREFIX . "product_filter pf" . $key . " ON (p2c.product_id = pf" . $key . ".product_id) LEFT JOIN " . DB_PREFIX . "filter_description fd" . $key . " ON (pf" . $key . ".filter_id = fd" . $key . ".filter_id)";
                    } else {
            ...
            if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
                ...
                if (!empty($data['filter_name'])) {
                    ...
                    foreach ($words as $word) {
                        $implode[] = "(pd.name LIKE '%" . $this->db->escape($word) . "%' OR cd.name LIKE '%" . $this->db->escape($word) . "%' OR fd.name LIKE '%" . $this->db->escape($word) . "%')";
                    }
            ...
            if (!empty($myCategories)) {
                ...
                foreach ($filter_groups as $key => $filters) {
                    if ($usingNamedFilters) {
                        $sql .= " AND fd" . $key . ".name IN ('" . implode("','", $filters) . "')";
                    } else {
                        $implode = array();
            ...
            if (!empty($data['filter_product_min_price'])) {
                $sql .= " AND IFNULL((SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int) $customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1),price) >= " . (float) $data['filter_product_min_price'];
            }
    ...
    public function getTotalProducts($data = array()) {
        ...
        if (is_null($product_data)) {
            ...
            if (!empty($myCategories)) {
                ...
                foreach ($filter_groups as $key => $filters) {
                    if ($usingNamedFilters) {
                        $sql .= " fd" . $key . ".name IN ('" . implode("','", $filters) . "') AND ";
                    } else {
                        $implode = array();
