      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" class="form-horizontal">
        <div class="form">
          <div class="control-group<?php if ($error_code) { ?> error<?php } ?>">
            ...
            <div class="controls">
							<textarea name="google_talk_code" class="span6 i-xxlarge" rows="4"><?php echo $google_talk_code; ?></textarea>
              <?php if ($error_code) { ?>
        ...
        <table id="module" class="list table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th class="column-or"><?php echo $entry_sort_order; ?></th>
              <th></th>
            </tr>
          </thead>
					<tbody>
              ...
              <td class="column-layout">
								...
								<select name="google_talk_module[<?php echo $module_row; ?>][layout_id]" class="span2 i-large">
              ...
              <td class="column-position">
								...
								<select name="google_talk_module[<?php echo $module_row; ?>][position]" class="span2 i-medium">
                  <?php echo p3html::oc_position_options($this->language, $module); ?>
                </select>
							</td>
              <td class="column-status">
								<label class="visible-480"><?php echo $entry_status; ?></label>
								<select name="google_talk_module[<?php echo $module_row; ?>][status]" class="input-small">
              ...
              <td class="column-sort">
								...
								<input type="text" name="google_talk_module[<?php echo $module_row; ?>][sort_order]" value="<?php echo $module['sort_order']; ?>" class="span1 i-mini">
							</td>
...
function addModule() {
	...
	html += '	<td class="column-layout"><label class="visible-480"><?php echo addslashes($entry_layout); ?></label><select name="google_talk_module[' + module_row + '][layout_id]" class="span2 i-large">';
	html += '		<?php echo p3html::oc_layout_options($layouts, null, true); ?>';
	html += '	</select></td>';
	html += '	<td class="column-position"><label class="visible-480"><?php echo addslashes($entry_position); ?></label><select name="google_talk_module[' + module_row + '][position]" class="span2 i-medium">';
	html += '		<?php echo p3html::oc_position_options($this->language, null, true); ?>';
	html += '	</select></td>';
	html += '	<td class="column-status"><label class="visible-480"><?php echo addslashes($entry_status); ?></label><select name="google_talk_module[' + module_row + '][status]" class="input-small">';
	html += '		<?php echo p3html::oc_status_options($this->language, null, true); ?>';
  html += '	</select></td>';
	html += '	<td class="column-sort"><label class="visible-480"><?php echo addslashes($entry_sort_order); ?></label><input type="text" name="google_talk_module[' + module_row + '][sort_order]" value="" class="span1 i-mini"></td>';
