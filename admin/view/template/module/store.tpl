...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="heading">
      <h1><i class="icon-shop"></i> <?php echo $heading_title; ?></h1>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" class="form-horizontal">
        <div class="form">
          <div class="control-group">
            <label class="control-label"><?php echo $entry_admin; ?></label>
            <div class="controls">
							<?php if ($this->config->get('p3adminrebooted_toggle_buttons')) { ?>
							<?php echo p3html::tb_bool_buttons_radio($this->language, 'store_admin', $store_admin); ?>
							<?php } else { ?>
               <?php echo p3html::tb_bool_radio_buttons($this->language, 'store_admin', $store_admin); ?>
							<?php } ?>
						</div>
          </div>
        </div>
        <table id="module" class="list table table-striped table-hover">
          ...
          <tbody>
              ...
              <td class="column-layout">
								...
								<select name="store_module[<?php echo $module_row; ?>][layout_id]" class="span2 i-large">
              ...
              <td class="column-position">
								...
								<select name="store_module[<?php echo $module_row; ?>][position]" class="span2 i-medium">
									<?php echo p3html::oc_position_options($this->language, $module); ?>
                </select>
							</td>
              <td class="column-status">
								<label class="visible-480"><?php echo $entry_status; ?></label>
								<select name="store_module[<?php echo $module_row; ?>][status]" class="input-small">
              ...
              <td class="column-sort">
								...
								<input type="text" name="store_module[<?php echo $module_row; ?>][sort_order]" value="<?php echo $module['sort_order']; ?>" class="span1 i-mini">
							</td>
...
function addModule() {
	...
	html += '	<td class="column-layout"><label class="visible-480"><?php echo addslashes($entry_layout); ?></label><select name="store_module[' + module_row + '][layout_id]" class="span2 i-large">';
	html += '		<?php echo p3html::oc_layout_options($layouts, null, true); ?>';
	html += '	</select></td>';
	html += '	<td class="column-position"><label class="visible-480"><?php echo addslashes($entry_position); ?></label><select name="store_module[' + module_row + '][position]" class="span2 i-medium">';
	html += '		<?php echo p3html::oc_position_options($this->language, null, true); ?>';
	html += '	</select></td>';
	html += '	<td class="column-status"><label class="visible-480"><?php echo addslashes($entry_status); ?></label><select name="store_module[' + module_row + '][status]" class="input-small">';
	html += '		<?php echo p3html::oc_status_options($this->language, null, true); ?>';
  html += '	</select></td>';
	html += '	<td class="column-sort"><label class="visible-480"><?php echo addslashes($entry_sort_order); ?></label><input type="text" name="store_module[' + module_row + '][sort_order]" value="" class="span1 i-mini"></td>';
