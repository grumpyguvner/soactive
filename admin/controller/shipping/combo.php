<?php
class ControllerShippingCombo extends Controller { 
	private $error = array();
	...
	public function index() {  
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('shipping/combo', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('shipping/combo', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['combo_modules'])) {
			$this->data['combo_modules'] = $this->request->post['combo_modules'];
		} else {
			$this->data['combo_modules'] = (array)$this->config->get('combo_modules');
		}

		if (isset($this->request->post['combo_tax_class_id'])) {
			$this->data['combo_tax_class_id'] = $this->request->post['combo_tax_class_id'];
		} else {
			$this->data['combo_tax_class_id'] = $this->config->get('combo_tax_class_id');
		}
		...
		$this->template = 'shipping/combo.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'shipping/combo')) {
